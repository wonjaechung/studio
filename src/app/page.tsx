...
export default function Home() {
    ...
    useEffect(() => {
        ...
        let appState = {
            ...
            calculator: { history: [] as { input: string, output: string, explanation?: string, type?: string, data?: any }[] },
            graphing: { pendingPlot: null as { indices: number[] } | null, currentView: 'plot' }
        };
        ...
        function getColumnData(name: string) { const col = getColumnByName(name); return col ? col.data.map(v => parseFloat(v)).filter(v => !isNaN(v)) : []; }
        ...
        const actions: { [key: string]: () => void } = {
            ...
            runInvNorm() { const area = parseFloat((document.getElementById('area') as HTMLInputElement).value); const mu = parseFloat((document.getElementById('mu') as HTMLInputElement).value); const sigma = parseFloat((document.getElementById('sigma') as HTMLInputElement).value); const result = stats.invNorm(area, mu, sigma); appState.calculator.history.push({ type: 'InvNorm', input: `invNorm(${area},${mu},${sigma})`, output: result.toFixed(6), data: { area, mu, sigma, result } }); renderCalculator(); closeModal(); },
            ...
            runBinomCdf() { const n = parseInt((document.getElementById('n') as HTMLInputElement).value); const p = parseFloat((document.getElementById('p') as HTMLInputElement).value); const x = parseInt((document.getElementById('x') as HTMLInputElement).value); let totalProb = 0; for (let i = 0; i <= x; i++) { totalProb += stats.binomialPdf(n, p, i); } appState.calculator.history.push({ type: 'BinomCdf', input: `binomCdf(${n},${p},${x})`, output: totalProb.toFixed(6), data: { n, p, x, result: totalProb } }); renderCalculator(); closeModal(); },
            ...
            runGeomPdf() { const p = parseFloat((document.getElementById('p') as HTMLInputElement).value); const x = parseInt((document.getElementById('x') as HTMLInputElement).value); const result = stats.geometricPdf(p, x); appState.calculator.history.push({ type: 'GeomPdf', input: `geomPdf(${p},${x})`, output: result.toFixed(6), data: { p, x, result } }); renderCalculator(); closeModal(); },
            ...
            runTInterval(mean: number, sx: number, n: number, cLevel: number) { const df = n - 1; const alpha = 1 - cLevel; const tStar = stats.invT(1 - alpha / 2, df); const me = tStar * (sx / Math.sqrt(n)); const lower = mean - me; const upper = mean + me; const output = `(${lower.toFixed(4)}, ${upper.toFixed(4)})`; appState.calculator.history.push({ type: 'tInterval', input: `tInterval(...)`, output: output, data: { lower, upper, me, df, mean, sx, n, cLevel } }); renderCalculator(); closeModal(); },
        ...
        function handleCalculatorInput() {
            ...
            if (expression === pythonCommand || expression === sqlCommand) {
                ...
                appState.calculator.history.push({ input: expression, output: "Success", explanation: "Sample dataset 'lab_data_1.csv' loaded into the spreadsheet." });
                commandHandled = true;
            } else if (expression.toLowerCase().includes('read_csv') || expression.toLowerCase().includes('select')) {
                appState.calculator.history.push({ input: expression, output: "Import Failed", explanation: "Incorrect query. Please copy the command from the 'Import Lab Data' panel exactly." });
                commandHandled = true;
            } else if (expression === 'df.head()') {
                if (appState.spreadsheet.isDataLoaded) {
                    renderDataFrameHead();
                    appState.calculator.history.push({ input: expression, output: "DataFrame head displayed in Viewer." });
                } else {
                    appState.calculator.history.push({ input: expression, output: "Error", explanation: "NameError: name 'df' is not defined. Load data first." });
                }
                commandHandled = true;
            }
            ...
            if (!commandHandled) {
                ...
                appState.calculator.history.push({ input: expression, output, explanation });
            }
